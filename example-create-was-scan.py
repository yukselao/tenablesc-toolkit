...
def main():
    ...
    for index, row in df.iterrows():
        ...
        if response.status == 200:
            logging.info(f"{scan_name} created successfully. http_status: {response.status}")
        else:
            if response.status == None and response.error == None and response.data == None:
                logging.warning("Check .env credentials and reload .env file")
            else:
                logging.error("Dataframe:{} , Error:{} ,Status: {} ".format(response.dataframe, response.error, response.status))
