...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'save' || $action === 'test') {
        if ($url === '' || $ak === '' || $sk === '') {
            $notice = 'URL, Access Key ve Secret Key zorunludur.';
            $noticeType = 'danger';
        } else {
            if ($action === 'save') {
                if (Db::saveSettings([
                    ...
                    $notice = 'Security Center ayarları kaydedildi.';
                    $current = Db::getSettings();
                } else {
                    $notice = 'Ayarlar kaydedilemedi (DB hatası: ' . h(Db::error()) . ').';
                    $noticeType = 'danger';
                }
            }
            ...
            try {
                ...
                $testResult = ['ok' => true, 'msg' => "Bağlantı başarılı — kullanıcı: $user"];
            } catch (Throwable $e) {
    ...
    <main class="content">
        <header class="page-head">
            ...
            <p class="text-secondary">
                Scan result'ları çekmek için Tenable Security Center bağlantı bilgilerini girin.
                Bilgiler veritabanında saklanır (lab kullanımı, kimlik doğrulama yok).
            </p>
        </header>
        ...
        <section class="card" style="max-width:720px;">
            <form method="post" class="card-body">
                <div class="mb-3">
                    ...
                    <div class="form-text">Örn: <code>https://&lt;host&gt;:8443</code> veya <code>https://&lt;host&gt;</code></div>
                    <div class="form-text text-warning">
                        <i class="bi bi-info-circle"></i>
                        Docker Desktop (Mac/Windows) altında SC bu makinede/loopback'te çalışıyorsa,
                        container'dan erişim için IP yerine <code>https://host.docker.internal:8443</code> kullanın.
                    </div>
                </div>
                <div class="mb-3">
                ...
                <div class="mb-3">
                    ...
                    <input type="password" name="sc_secret_key" class="form-control font-monospace"
                           placeholder="<?= $secretSet ? '•••••••• (kayıtlı — değiştirmek için yeni değer girin)' : '' ?>"
                           <?= $secretSet ? '' : 'required' ?>>
                    <?php if ($secretSet): ?>
                        <div class="form-text text-success"><i class="bi bi-check2"></i> Secret key kayıtlı.</div>
                    <?php endif; ?>
                </div>
